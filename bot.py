...
Sources:
  ...
  🔴 YouTube   — direct URL
  🔍 Search    — song name (searches YouTube + JioSaavn)

Quality: user picks 128 kbps or 320 kbps via inline buttons.
Metadata: title, artist, album, cover art, lyrics embedded in every MP3.
Health:   /health endpoint for Koyeb.
"""
...
async def send_audio(
    ...
    """Send MP3 file as Telegram Audio message."""
...
async def run_spotify(
    ...
    try:
        if kind == "track":
            tracks = spotify_track(url)
        elif kind == "album":
            tracks = spotify_album(url)
        elif kind == "playlist":
            tracks = spotify_playlist(url)
        elif kind == "artist":
            tracks = spotify_artist(url)
        else:
            tracks = []

        if not tracks:
            await status.edit_text(
                "❌ Spotify returned no results.\n"
        ...
        for track in tracks[:MAX_PLAYLIST_SONGS]:
            ...
            try:
                ...
                log.error(f"Spotify track error '{track.get('title')}': {type(e).__name__}: {e}")
                await context.bot.send_message(
                    chat_id,
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "🔴 YouTube — direct URL\n"
        "🔍 Any song name — I'll search for it\n\n"
        "You'll get to choose *128 kbps* or *320 kbps* quality.\n"
...
async def on_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Store payload in bot_data keyed by message_id
